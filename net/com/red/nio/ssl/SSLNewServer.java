...
public class SSLNewServer {
	...
	private void createSSLContext() throws GeneralSecurityException,
	...
	// create the server socket, bind it to port 1234, set unblock and register
	// the "accept" only
		...
		if (key.isAcceptable()) {
			...
			try {
				SocketChannel sc = ((ServerSocketChannel) key.channel())
						.accept();
				doHandShake(sc);// if it is an accept event, do the handshake in
								// a blocking mode
	...
	/*
	 * public static HandshakeStatus runDelegatedTasks(SSLEngineResult
	 * engineResult, SSLEngine sslEngine) { if
	 * (engineResult.getHandshakeStatus() == HandshakeStatus.NEED_TASK) {
	 * Runnable runnable; while ((runnable = sslEngine.getDelegatedTask()) !=
	 * null) { System.out.println("\trunning delegated task...");
	 * runnable.run(); } HandshakeStatus hsStatus =
	 * sslEngine.getHandshakeStatus(); if (hsStatus ==
	 * HandshakeStatus.NEED_TASK) { //throw new
	 * Exception("handshake shouldn't need additional tasks");
	 * System.out.println("handshake shouldn't need additional tasks"); }
	 * System.out.println("\tnew HandshakeStatus: " + hsStatus); } return
